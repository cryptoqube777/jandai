...
contract Jandai is Context, IBEP20, Ownable {
    ...
    string private     _name = "Jandai";
    string private   _symbol = "JAND";
    ...
    uint256 public _antiWhaleThreshold = _tTotal.div(100);
    ...
    constructor (address cOwner, address tWallet) Ownable(cOwner) {
    ...
    function setAntiWhaleThreshold(uint16 percentage) external onlyOwner {
        require(percentage <= 1000, "Antiwhale percentage cannot exceed 1000 (100 * 10)");
        _antiWhaleThreshold = _tTotal.mul(percentage).div(1000);
    }
    ...
    function _takeLiquidity(uint256 tLiquidity, address account) private {
        ...
        //team tax
        _rOwned[_teamWallet] = _rOwned[_teamWallet].add(rLiquidity);
        if (_isExcluded[_teamWallet]) {
            _tOwned[_teamWallet] = _tOwned[_teamWallet].add(tLiquidity);
        }
        emit Transfer(account, _teamWallet, tLiquidity);

        // burn
        _rTotal = _rTotal.sub(rLiquidity);
        _tTotal = _tTotal.sub(tLiquidity);
        emit Transfer(account, address(0), tLiquidity);
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity.mul(3));
                        ...
                        private pure returns (uint256, uint256, uint256)
    {
        ...
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity.mul(3));
